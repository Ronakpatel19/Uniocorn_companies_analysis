...
-- 11. Calculate the average valuation of unicorn companies.
SELECT AVG(Valuation) AS Avg_Valuation 
FROM unicorn_companies;

-- 12. Find the industry with the highest total valuation.
...
-- 13. Find the top 3 countries with the most unicorn companies.
...
-- 14. Find the funding-to-valuation ratio for each company.
SELECT Company, (Funding / Valuation) AS Funding_To_Valuation_Ratio 
FROM unicorn_companies;

-- 15. Identify companies with valuations greater than 10 times their funding.
SELECT * 
FROM unicorn_companies 
WHERE Valuation > 10 * Funding;

-- 16. Find the youngest unicorns based on the year founded.
SELECT * 
FROM unicorn_companies 
ORDER BY Year_founded DESC 
LIMIT 10;

-- 17. Identify industries with an average valuation above $5B.
SELECT Industry, AVG(Valuation) AS Avg_Valuation 
FROM unicorn_companies 
GROUP BY Industry 
HAVING Avg_Valuation > 5000000000;

-- 18. Calculate the time taken for each company to become a unicorn.
SELECT Company, (YEAR(Date_joined) - Year_founded) AS Years_To_Unicorn 
FROM unicorn_companies;

-- 19. Find the investors who funded the highest number of unicorns.
...
-- 20. Identify countries where unicorns have the shortest time to reach unicorn status.
...
-- 21. Find the year with the highest number of new unicorns.
...
-- 22. Find the company that took the longest time to become a unicorn.
SELECT Company, (YEAR(Date_joined) - Year_founded) AS Years_To_Unicorn 
FROM unicorn_companies 
ORDER BY Years_To_Unicorn DESC 
LIMIT 1;

-- 23. Calculate the total valuation of all unicorns per continent.
SELECT Continent, SUM(Valuation) AS Total_Valuation 
FROM unicorn_companies 
GROUP BY Continent;

-- 24. Find unicorns that are in the top 10% based on valuation.
SELECT * 
FROM unicorn_companies 
WHERE Valuation >= (SELECT PERCENTILE_CONT(0.9) WITHIN GROUP (ORDER BY Valuation) FROM unicorn_companies);

-- 25. Identify the fastest-growing industries based on the average time to unicorn status.
...
-- 26. Find the company with the highest valuation-to-funding ratio.
SELECT Company, (Valuation / Funding) AS Valuation_To_Funding_Ratio 
FROM unicorn_companies 
ORDER BY Valuation_To_Funding_Ratio DESC 
LIMIT 1;

-- 27. Find the most common year when unicorns were founded.
...
-- 28. List companies with valuations higher than the industry average.
...
-- 29. Find investors who funded unicorns in multiple industries.
...
-- 30. Identify unicorns that were founded after 2015 and became unicorns within 3 years.
SELECT * 
FROM unicorn_companies 
